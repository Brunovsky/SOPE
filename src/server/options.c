...
static const struct option long_options[] = {
    ...
    {DELAY_LFLAG,       required_argument, NULL,         DELAY_FLAG},
    // end of options
    {0, 0, 0, 0}
...
static const wchar_t* usage = L"usage: server [option]... seats threads time\n"
    ...
    "  -d, --delay=T         Set DELAY() to nanosleep for T\n"
    "                        milliseconds (if unchanged)\n"
    "\n";
...
int parse_args(int argc, char** argv) {
    ...
    while (true) {
        ...
        case CLIENTMAX_FLAG:
            ...
            break;
        case DELAY_FLAG:
            if (parse_int(optarg, &o_delay) != 0) {
                print_badarg(DELAY_LFLAG);
            }
            break;
        case '?':
        case ':':
